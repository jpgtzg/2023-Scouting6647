...
class NotesController {
  ...
  Future<List<Notes>?> getAll() async {
    await init();
    final notes = (await sheet.values.map.allRows());
    return notes?.map((json) => Notes.fromGsheets(json)).toList();
  }

  Future<Notes?> getById(int id) async {
    await init();
    final map = await sheet.values.map.rowByKey(
      id,
      fromColumn: 1,
    );
    return map == null ? null : Notes.fromGsheets(map);
  }

  Future<List<Notes>> getAllByID(int id) async {
    await init();

    //GETS ALL ROWS
    final notes = (await sheet.values.map.allRows());
    final allrows = notes!.map((json) => Notes.fromGsheets(json)).toList();

    List<Notes> rows = <Notes>[];
  ...
  Future<void> insertData(Notes notes) async {
    await init();
    final list = notes.toGsheets();
    await sheet.values.map.appendRow(list);

    final convertedList = notes.toList();
